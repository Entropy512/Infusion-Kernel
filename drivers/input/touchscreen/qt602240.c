...
void qt_Acquisition_Config_Init(void)
{
	...
	acquisition_config.chrgtime = 10; // 4.52inch 
#else
...
#elif defined(CONFIG_HAWK_VER_B1_REAL_ADDED_FEATURE) 
    gripfacesuppression_config.szthr1 = 30;
    gripfacesuppression_config.szthr2 = 20;
    gripfacesuppression_config.shpthr1 = 4;
    gripfacesuppression_config.shpthr2 = 15;
...
extern uint8_t read_dbg_data(uint8_t dbg_mode , uint8_t node, uint16_t * dbg_data);
#endif
...
#ifdef CONFIG_S5PC110_DEMPSEY_BOARD
	/* check reference for only STEALTH-V
		 If reference value is more than 13500, change gain of touchscreen config (2->1)
	   */
	
	  read_dbg_data(QT_REFERENCE_MODE, 208, &qt_refrence);
	  printk("\n[TSP] current reference : %d\n", qt_refrence);
	
	  if( qt_refrence > 12500 ) {
		  printk("[TSP] detect refrence is high \n");
	
		  touchscreen_config.blen = 16;   /* set gain 1 */
		  ...
		  noise_suppression_config.freq[0] = 48;  
		  noise_suppression_config.freq[1] = 55;  
		  noise_suppression_config.freq[2] = 61;  
		  noise_suppression_config.freq[3] = 66;  
		  noise_suppression_config.freq[4] = 69;  
...
static unsigned int last_status_of_tsp_for_ta_detect=0;	
// 0 : VAC (TA, USB) Detached. 1 : VAC (TA, USB) Attached.	
// In order to check whether either TA or USB connected or not, 
#endif
...
int set_tsp_for_tvout_35pi_detect(int state)
{
	...
	if ( last_status_of_tsp_for_ta_detect == 1)
	{
		return -1;
	}
...
{
	int i = 0, temp_value = 0; 	
	uint8_t status; 
	int ret = WRITE_MEM_OK; 		
	

	printk("[%s] palm_touch_pressed = %d...\n", __func__, palm_touch_pressed);
	...
	#endif
		{
			...
			if( quantum_msg[0] == 14 )
			{
				...
				{ 
					...
					input_sync(qt602240->input_dev);
#if defined(CONFIG_S5PC110_DEMPSEY_BOARD)	
					if(palm_touch_pressed){
							hrtimer_cancel(&palmtimer);
							palm_touch_pressed = false;
							printk("stop timer palm_touch_pressed false\n");
					}
#endif
...
//						calibrate_chip();
	...
	if(!palm_touch_pressed)
	{
						 ...
						 printk("set timer for palmtimer\n");
...
//					cal_check_flag = 2u;
				...
				{	
					if ( touch_state_val == 1 )
					{
						...
						{
							printk("[TSP Key] Key Release (0x%x), but blocked due to TSP active.\n", quantum_msg[2]);
						}
					}
					else
					{
...
//dempsey
		...
		// Turn CAM_ISP_1.8V on  //touch
//		Set_MAX8998_PM_OUTPUT_Voltage(LDO14, VCC_1p800);VCC_2p800
		...
		Set_MAX8998_PM_OUTPUT_Voltage(LDO15, VCC_3p300);
